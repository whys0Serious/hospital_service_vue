  ...
  <div>
    <el-form :model="docschedule" label-width="20%" style="width: 40%">
      <el-form-item label="医生姓名">
        <el-input v-model="docschedule.docName"></el-input>
      </el-form-item>
      <el-form-item label="活动科室" v-model="docschedule.docName">
...
<script>
  ...
  export default{
    ...
    methods:{
      finddepart(){
        axios.get("api/houtai/findalldocsebyid/"+this.$route.query.id).then(res=>{
//          this.docschedule.department=res.data
          this.docschedule.depart=res.data.department
          this.docschedule.docName=res.data.docName
        })
      },
      onSubmit(formName) {
